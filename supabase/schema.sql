...
-- =================================================================
-- L'ARBITRE DU CHEVAL DE TROIE
-- =================================================================
-- CE QUE CE BLOC RÉSOUT, ET POURQUOI IL EXISTE.
--
-- La variante « Cheval de Troie » (js/troie-*.js) repose sur une information
-- cachée : chaque joueur a choisi un cavalier ADVERSE pour être son espion, et
-- l'autre ne sait pas lequel. En ligne, le choix ne traverse jamais le réseau
-- — il n'existe que dans le navigateur qui l'a fait. C'est ce qui rend le
-- secret réel plutôt que promis.
--
-- Mais un secret que personne d'autre ne connaît, personne d'autre ne peut le
-- vérifier. Deux coups reposent sur lui et sur lui seul :
--   · LA RÉVÉLATION : « ce cavalier à toi était mon espion, il change de
--     camp » ;
--   · LE COUP QUI NE TIENT QUE PAR LE SECRET : mon roi a l'air en prise d'un
--     de tes cavaliers, mais ce cavalier est mon espion — il n'attaque rien.
-- Sans arbitre, le client d'en face n'a d'autre choix que de CROIRE : il
-- accepte le coup dès qu'une hypothèse le rend légal. Un client bricolé peut
-- donc ignorer un échec une fois, en désignant un cavalier au hasard.
--
-- CE SERVEUR TRANCHE EXACTEMENT CETTE QUESTION-LÀ, ET RIEN D'AUTRE. Il ne
-- connaît ni le plateau, ni les coups, ni les règles des échecs : il détient
-- les deux chevaux SCELLÉS, et il répond à une seule question — « la pièce que
-- ce joueur invoque est-elle bien celle qu'il a scellée au coup d'envoi ? ».
-- C'est la seule chose qu'un serveur peut vérifier ici sans rejouer la partie,
-- et c'est précisément ce qui manquait.
--
-- -- POURQUOI ÇA NE FUITE PAS ---------------------------------------
-- 1. LA TABLE EST ILLISIBLE. RLS activé, aucune policy : la clé publishable
--    du jeu ne peut pas lire une seule ligne. Les fonctions ci-dessous ne
--    renvoient JAMAIS la pièce scellée — seulement des booléens.
-- 2. ON NE RÉPOND QU'À UNE QUESTION DÉJÀ POSÉE. ec_troie_verify ne dit « oui »
--    que s'il existe une RÉCLAMATION en cours de l'adversaire, sur CETTE
--    pièce, à CE demi-coup. Sans réclamation, la réponse est « non » quelle
--    que soit la pièce : sonder les deux cavaliers d'en face n'apprend rien.
-- 3. RÉCLAMER, C'EST SE DÉCOUVRIR. ec_troie_claim n'accepte que la vraie
--    pièce scellée : un client bricolé ne peut pas produire de réclamation
--    valable, donc son coup sera refusé en face. Et le joueur honnête qui
--    réclame a de toute façon révélé son espion par son coup — c'est la règle
--    elle-même qui le veut, pas le serveur qui le trahit.
--
-- -- CE QUE ÇA NE FAIT PAS ------------------------------------------
-- Le serveur ne valide pas les coups : il ne sait pas jouer aux échecs, et
-- lui apprendre demanderait d'y porter tout le moteur. Les deux clients
-- continuent donc de vérifier la géométrie, la propriété des pièces et la
-- légalité (troRemoteOptions, js/troie-rules.js) ; le serveur ne tranche que
-- ce qu'eux ne peuvent pas trancher — le secret.
-- =================================================================

create table if not exists public.ec_troie_seals(
  code         text        not null,   -- le salon de la partie (epichess-troie-<code>)
  player       text        not null,   -- l'identifiant réseau du joueur (éphémère)
  piece        text        not null,   -- la pièce scellée : « p12 ». JAMAIS renvoyée.
  claim_piece  text,                   -- réclamation en cours, s'il y en a une
  claim_ply    integer,
...
alter table public.ec_troie_seals enable row level security;
-- Aucune policy, et c'est tout le propos : la table est hors d'atteinte du
-- client. Seules les fonctions SECURITY DEFINER ci-dessous y touchent.

-- Une réclamation ne vaut que quelques secondes : le temps qu'un coup
-- traverse le réseau. Au-delà, elle est périmée — ce qui empêche de rejouer
-- une vieille réclamation pour faire passer un coup d'aujourd'hui.
create or replace function public.ec_troie_claim_window() returns interval
language sql immutable as $$ select interval '90 seconds' $$;

-- SCELLER SON CHEVAL, une fois pour toutes. Le rejeu à l'identique est permis
-- (un message perdu, un onglet rechargé) ; changer de pièce ne l'est pas —
-- sans quoi le secret ne serait plus un engagement, et on pourrait choisir son
-- espion après coup, en fonction de ce qui arrange.
create or replace function public.ec_troie_seal(
  p_code text, p_player text, p_piece text)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_old text;
begin
  if p_code is null or p_player is null or p_piece is null then
    raise exception 'Requête incomplète.';
  end if;
  -- Ménage : les parties d'hier n'intéressent plus personne.
  delete from ec_troie_seals where created_at < now() - interval '1 day';
  select piece into v_old from ec_troie_seals
    where code = p_code and player = p_player;
  if v_old is null then
    insert into ec_troie_seals(code, player, piece) values (p_code, p_player, p_piece);
    return jsonb_build_object('ok', true, 'sealed', true);
  end if;
  return jsonb_build_object('ok', v_old = p_piece, 'sealed', true);
end $$;

-- RÉCLAMER SON CHEVAL pour le coup qu'on s'apprête à envoyer. Le serveur
-- répond « oui » uniquement si la pièce invoquée est celle qui a été scellée,
-- et n'enregistre la réclamation que dans ce cas : une réclamation mensongère
-- ne laisse aucune trace, donc l'adversaire ne trouvera rien à vérifier et
-- refusera le coup.
create or replace function public.ec_troie_claim(
  p_code text, p_player text, p_piece text, p_ply integer)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_piece text;
begin
  select piece into v_piece from ec_troie_seals
    where code = p_code and player = p_player;
  if v_piece is null then
    return jsonb_build_object('ok', false, 'reason', 'unsealed');
  end if;
  if v_piece <> p_piece then
    return jsonb_build_object('ok', false, 'reason', 'mismatch');
  end if;
  update ec_troie_seals
     set claim_piece = p_piece, claim_ply = p_ply, claimed_at = now()
   where code = p_code and player = p_player;
  return jsonb_build_object('ok', true);
end $$;

-- VÉRIFIER LA RÉCLAMATION DE L'ADVERSAIRE. On ne demande pas « quel est son
-- cheval ? » — cette question-là n'a pas de réponse ici. On demande « a-t-il
-- prouvé, à l'instant et pour ce demi-coup, que c'est CETTE pièce ? », et la
-- réponse est non par défaut.
create or replace function public.ec_troie_verify(
  p_code text, p_player text, p_piece text, p_ply integer)
returns jsonb language plpgsql security definer set search_path = public as $$
declare r record;
begin
  select claim_piece, claim_ply, claimed_at into r from ec_troie_seals
    where code = p_code and player = p_player;
  if r.claim_piece is null then
    return jsonb_build_object('ok', false);
  end if;
  return jsonb_build_object('ok',
    r.claim_piece = p_piece
    and (p_ply is null or r.claim_ply = p_ply)
    and r.claimed_at > now() - ec_troie_claim_window());
end $$;
...
grant execute on function
  ...
  public.ec_troie_seal(text, text, text),
  public.ec_troie_claim(text, text, text, integer),
  public.ec_troie_verify(text, text, text, integer)
